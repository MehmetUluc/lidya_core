class LidyaPos {
    constructor() {
        ...
        LidyaPos.handleCounterUp();
        LidyaPos.initMediaIntegrate();
        if (LidyaPosVariables && LidyaPosVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = LidyaPosVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = LidyaPosVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            LidyaPos.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        LidyaPos.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    LidyaPos.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            LidyaPos.showError(item);
                        });
                    });
                }
            } else {
                LidyaPos.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + LidyaPosVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        LidyaPos.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            LidyaPos.callScroll($('.list-item-checkbox'));
        }
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = LidyaPos.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', LidyaPos.onDragTool);
            $(window).on('mouseup', LidyaPos.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', LidyaPos.onDragTool);
        $(window).off('mouseup', LidyaPos.onReleaseTool);
    }
